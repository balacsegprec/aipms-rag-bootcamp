...
def precision_at_k(retrieved_chunks, expected_text, k=TOP_K):
...
def run_evaluation(queries, tenant_id='default', entity_type=None, contract_standard=None):
    baseline_scores = []
    hyde_scores = []
    multi_scores = []
    ...
    for item in queries:
        ...
        baseline_scores.append(precision_at_k(baseline_res, expected))
        ...
        if hyde_doc and not hyde_doc.startswith('[ERROR]'):
            ...
            hyde_res = []
        hyde_scores.append(precision_at_k(hyde_res, expected))
        ...
        multi_scores.append(precision_at_k(multi_res, expected))
